...
b. By setting the cookie values via javascript, the attacker could change the user's theme (by modifying the theme cookie) or log the user out by clearing the `session` cookie. They could also store the `session` cookie and submit it in a new HTTP POST request on the server to allow anyone to impersonate the user.
